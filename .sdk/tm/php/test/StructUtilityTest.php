...
// Evervault SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EvervaultOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EvervaultOmni::structfix($now, Struct::undef()))
                        !== json_encode(EvervaultOmni::structfix($val, Struct::undef()))) {
