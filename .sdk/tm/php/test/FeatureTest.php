...
class FtCtrl extends EvervaultControl
{
...
class FtClient
{
    ...
    private EvervaultUtility $utility;

    public function __construct(EvervaultUtility $utility, array $options)
    {
    ...
    public function get_utility(): EvervaultUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EvervaultContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EvervaultContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EvervaultContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EvervaultConfig::make_feature($name);
    ...
    private static function buildUrl(EvervaultSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EvervaultError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EvervaultResponse($fetched);
        }
    ...
    private function fail(EvervaultContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EvervaultContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
