...
{
    private static function utility(): EvervaultUtility
    {
        return new EvervaultUtility();
    }
    ...
    private static function ctx(array $over = []): EvervaultContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EvervaultContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EvervaultOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EvervaultError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EvervaultOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EvervaultMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EvervaultOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EvervaultMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EvervaultOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EvervaultMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EvervaultSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EvervaultMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EvervaultResponse([]),
            'result' => new EvervaultResult([]),
        ]);
        [, $err] = EvervaultMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EvervaultSpec([]),
            'response' => null,
            'result' => new EvervaultResult([]),
        ]);
        [, $err] = EvervaultMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EvervaultSpec(['step' => 's']),
            'response' => new EvervaultResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EvervaultResult([]),
        ]);
        [, $err] = EvervaultMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EvervaultSpec(['step' => 's']),
            'response' => new EvervaultResponse(self::resp(200, ['v' => 1])),
            'result' => new EvervaultResult([]),
        ]);
        [, $err] = EvervaultMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EvervaultSpec(['step' => 's']),
            'response' => new EvervaultResponse(self::resp(200, ['v' => 2])),
            'result' => new EvervaultResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EvervaultMakeResponse::call($ctx);
    ...
    {
        $preset = new EvervaultResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EvervaultSpec([]),
            'response' => new EvervaultResponse([]),
            'result' => new EvervaultResult([]),
        ...
        [$got, $err] = EvervaultMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EvervaultResult([])]);
        [, $err] = EvervaultMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EvervaultSpec([]), 'result' => null]);
        [, $err] = EvervaultMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EvervaultSpec(['step' => 's']),
            'result' => new EvervaultResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EvervaultOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EvervaultMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EvervaultSpec(['step' => 's']),
            'result' => new EvervaultResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EvervaultOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EvervaultMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EvervaultResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EvervaultSpec([]),
            'result' => new EvervaultResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EvervaultMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EvervaultMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EvervaultError('boom', 'boom');
        $utility->fetcher = function (EvervaultContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EvervaultSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EvervaultMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EvervaultContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EvervaultMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EvervaultResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EvervaultContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EvervaultSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EvervaultMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EvervaultResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EvervaultSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EvervaultMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EvervaultMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EvervaultMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EvervaultResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EvervaultDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EvervaultResult(['ok' => false])]);
        $this->expectException(EvervaultError::class);
        EvervaultDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EvervaultResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EvervaultMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EvervaultMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EvervaultBaseFeature();
        $b = new EvervaultBaseFeature();
        EvervaultFeatureAdd::call($ctx, $a);
        EvervaultFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EvervaultBaseFeature
    {
        $f = new EvervaultBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EvervaultFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EvervaultContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EvervaultSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EvervaultPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EvervaultResponse(['headers' => null]),
            'result' => new EvervaultResult([]),
        ]);
        EvervaultResultHeaders::call($ctx);
