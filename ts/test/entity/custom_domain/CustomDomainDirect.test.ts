...
describe('CustomDomainDirect', async () => {
  ...
  test('direct-load-custom_domain', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-custom_domain', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","relay_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EVERVAULT_TEST_CUSTOM_DOMAIN_ENTID']
