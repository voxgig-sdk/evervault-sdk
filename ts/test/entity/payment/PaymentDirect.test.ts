...
describe('PaymentDirect', async () => {
  ...
  test('direct-list-payment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-payment', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EVERVAULT_TEST_PAYMENT_ENTID']
