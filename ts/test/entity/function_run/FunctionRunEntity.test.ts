...
describe('FunctionRunEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'function_run.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['function_run01','function_run02','function_run03','function01','function02','function03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['EVERVAULT_TEST_FUNCTION_RUN_ENTID']
