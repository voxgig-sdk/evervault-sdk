...
describe('WebhookEndpointDirect', async () => {
  ...
  test('direct-load-webhook_endpoint', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-webhook_endpoint', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EVERVAULT_TEST_WEBHOOK_ENDPOINT_ENTID']
