...
describe('ThreeDsSessionDirect', async () => {
  ...
  test('direct-load-three_ds_session', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-three_ds_session', setup.live)) return
    if (skipIfMissingIds(t, setup, ["3ds_session_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['EVERVAULT_TEST_THREE_DS_SESSION_ENTID']
