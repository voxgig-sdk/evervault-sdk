...
function isRunnable(code) {
    return (/new\s+EvervaultSDK\b/.test(code) ||
        /\bEvervaultSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EvervaultSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EvervaultSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
