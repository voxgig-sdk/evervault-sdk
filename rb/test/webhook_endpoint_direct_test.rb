...
class WebhookEndpointDirectTest < Minitest::Test
  def test_direct_load_webhook_endpoint
    setup = webhook_endpoint_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-webhook_endpoint", setup[:live] ? "live" : "unit")
    if _should_skip
...
def webhook_endpoint_direct_setup(mockres)
