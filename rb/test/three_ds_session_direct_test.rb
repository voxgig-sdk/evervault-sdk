...
class ThreeDsSessionDirectTest < Minitest::Test
  def test_direct_load_three_ds_session
    setup = three_ds_session_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-three_ds_session", setup[:live] ? "live" : "unit")
    if _should_skip
...
def three_ds_session_direct_setup(mockres)
