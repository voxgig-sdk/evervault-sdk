...
class CustomDomainDirectTest < Minitest::Test
  def test_direct_load_custom_domain
    setup = custom_domain_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-custom_domain", setup[:live] ? "live" : "unit")
    if _should_skip
...
def custom_domain_direct_setup(mockres)
