...
class ThreeDsSessionEntityTest < Minitest::Test
  def test_create_instance
    testsdk = EvervaultSDK.test(nil, nil)
    ent = testsdk.ThreeDsSession(nil)
    assert !ent.nil?
  end

  def test_basic_flow
    setup = three_ds_session_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "three_ds_session." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_THREE_DS_SESSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    three_ds_session_ref01_ent = client.ThreeDsSession(nil)
...
def three_ds_session_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "three_ds_session", "ThreeDsSessionTestData.json")
  ...
  entid_env_raw = ENV["EVERVAULT_TEST_THREE_DS_SESSION_ENTID"]
