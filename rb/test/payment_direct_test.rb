...
class PaymentDirectTest < Minitest::Test
  def test_direct_list_payment
    setup = payment_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-payment", setup[:live] ? "live" : "unit")
    if _should_skip
...
def payment_direct_setup(mockres)
