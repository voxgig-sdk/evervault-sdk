...
class BinLookupEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = bin_lookup_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "bin_lookup." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_BIN_LOOKUP_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    bin_lookup_ref01_ent = client.BinLookup(nil)
...
def bin_lookup_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "bin_lookup", "BinLookupTestData.json")
  ...
  entid_env_raw = ENV["EVERVAULT_TEST_BIN_LOOKUP_ENTID"]
