...
class ClientSideTokenEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = client_side_token_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "client_side_token." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_CLIENT_SIDE_TOKEN_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    client_side_token_ref01_ent = client.ClientSideToken(nil)
...
def client_side_token_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "client_side_token", "ClientSideTokenTestData.json")
  ...
  entid_env_raw = ENV["EVERVAULT_TEST_CLIENT_SIDE_TOKEN_ENTID"]
