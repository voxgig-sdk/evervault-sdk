...
class ThreeDsSessionEntity
  ...
  def make
    opts = @_entopts.dup
    ThreeDsSessionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EvervaultHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ThreeDsSession, Hash] the current ThreeDsSession data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EvervaultHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ThreeDsSession fields)
  def match_get
  ...
  #
  # @param reqmatch [ThreeDsSessionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ThreeDsSession.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ThreeDsSession, Hash] the loaded ThreeDsSession; raises EvervaultError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EvervaultHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqdata [ThreeDsSessionCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [ThreeDsSession, Hash] the created ThreeDsSession; raises EvervaultError on failure
