...
module EvervaultUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EvervaultResponse.new({})
    ...
    if fetch_err
      ...
      response = EvervaultResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EvervaultResponse.new(fetched)
    else
