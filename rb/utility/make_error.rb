...
module EvervaultUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = EvervaultContext.new({}, nil)
    end
    op = ctx.op || EvervaultOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || EvervaultResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(EvervaultError) ? err.msg : err.to_s
    ...
    sdk_err = EvervaultError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(EvervaultError)
