...
class TestCardArtEntity:

    def test_should_create_instance(self):
        testsdk = EvervaultSDK.test(None, None)
        ent = testsdk.CardArt(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _card_art_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "card_art." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set EVERVAULT_TEST_CARD_ART_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        card_art_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.card_art")))
        card_art_ref01_data = None
        if len(card_art_ref01_data_raw) > 0:
            card_art_ref01_data = helpers.to_map(card_art_ref01_data_raw[0][1])

        # LOAD
        card_art_ref01_ent = client.CardArt(None)
        card_art_ref01_match_dt0 = {}
        card_art_ref01_data_dt0_loaded = card_art_ref01_ent.load(card_art_ref01_match_dt0, None)
        assert card_art_ref01_data_dt0_loaded is not None



def _card_art_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/card_art/CardArtTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("EVERVAULT_TEST_CARD_ART_ENTID"))
