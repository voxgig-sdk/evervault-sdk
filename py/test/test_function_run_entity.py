...
class TestFunctionRunEntity:

    def test_should_create_instance(self):
        testsdk = EvervaultSDK.test(None, None)
        ent = testsdk.FunctionRun(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _function_run_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "function_run." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set EVERVAULT_TEST_FUNCTION_RUN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        function_run_ref01_ent = client.FunctionRun(None)
        ...
        function_run_ref01_data = helpers.to_map(runner.entity_data(function_run_ref01_ent.create(function_run_ref01_data, None)))
        assert function_run_ref01_data is not None
        assert function_run_ref01_data["id"] is not None



def _function_run_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/function_run/FunctionRunTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "EVERVAULT_TEST_FUNCTION_RUN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("EVERVAULT_TEST_FUNCTION_RUN_ENTID"))
