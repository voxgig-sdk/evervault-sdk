...
class TestCustomDomainDirect:

    def test_should_direct_load_custom_domain(self):
        setup = _custom_domain_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-custom_domain", "live" if setup["live"] else "unit")
        if _skip:
...
def _custom_domain_direct_setup(mockres):
