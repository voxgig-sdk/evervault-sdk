...
class TestRelayDirect:

    def test_should_direct_load_relay(self):
        setup = _relay_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-relay", "live" if setup["live"] else "unit")
        if _skip:
...
def _relay_direct_setup(mockres):
