...
class TestThreeDsSessionDirect:

    def test_should_direct_load_three_ds_session(self):
        setup = _three_ds_session_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-three_ds_session", "live" if setup["live"] else "unit")
        if _skip:
...
def _three_ds_session_direct_setup(mockres):
