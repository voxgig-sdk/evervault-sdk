...
class TestCardDirect:
    ...
    def test_should_direct_load_card(self):
        setup = _card_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-card", "live" if setup["live"] else "unit")
        if _skip:
