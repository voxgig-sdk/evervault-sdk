...
class TestWebhookEndpointDirect:

    def test_should_direct_load_webhook_endpoint(self):
        setup = _webhook_endpoint_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-webhook_endpoint", "live" if setup["live"] else "unit")
        if _skip:
...
def _webhook_endpoint_direct_setup(mockres):
