...
class TestMerchantDirect:
    ...
    def test_should_direct_load_merchant(self):
        setup = _merchant_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-merchant", "live" if setup["live"] else "unit")
        if _skip:
