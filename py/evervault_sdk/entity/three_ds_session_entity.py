...
from evervault_sdk.evervault_types import (
    ThreeDsSession,
    ThreeDsSessionLoadMatch,
    ThreeDsSessionCreateData,
)
...
class ThreeDsSessionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ThreeDsSessionEntity(self._client, opts)
    ...
    def data_get(self) -> ThreeDsSession:
    ...
    def match_get(self) -> ThreeDsSession:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ThreeDsSession:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ThreeDsSession().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: ThreeDsSessionCreateData, ctrl=None) -> ThreeDsSession:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
