...
from evervault_sdk.core.helpers import get_ctx_prop, to_map
...
class EvervaultContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, EvervaultSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, EvervaultResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, EvervaultResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return EvervaultError(code, msg, self)
