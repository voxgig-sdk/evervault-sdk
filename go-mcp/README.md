...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/evervault-mcp)
make build
...
claude mcp add --scope user evervault \
  -- /absolute/path/to/evervault-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./evervault-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/evervault-mcp
   ```
...
```sh
./evervault-mcp -transport http -addr :18080 &
