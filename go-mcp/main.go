// Command Evervault-mcp is an MCP server exposing the Evervault
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Evervault_list  — list records of an entity
//	Evervault_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: EVERVAULT_APIKEY carries the API key and
	// EVERVAULT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EVERVAULT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EVERVAULT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEvervaultSDK(opts)
