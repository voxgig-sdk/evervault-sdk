...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Card()` — with named operations (`load`/`create`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
// create() returns the ENTITY — call data_get() for the created Card record.
$created = $client->Card()->create(["expiry" => [], "month" => "example_month", "number" => "example_number", "year" => "example_year"]);
...
// $card->data_get() now returns the card data from the last load
