...
// Evervault SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class EvervaultMakeError
{
    public static function call(?EvervaultContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EvervaultContext([], null);
        }
        $op = $ctx->op ?? new EvervaultOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EvervaultResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EvervaultError) ? $err->msg : (string)$err;
        $msg = "EvervaultSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new EvervaultError('', $msg, $ctx);
        ...
        if ($err instanceof EvervaultError) {
