...
class AcquirerEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AcquirerEntity($this->_client, $opts);
    }

    /**
     * @param Acquirer|array $args Acquirer data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EvervaultHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Acquirer|array The current Acquirer data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Acquirer fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EvervaultHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Acquirer fields).
     */
    ...
    /**
     * Load a single Acquirer.
     *
     * @param AcquirerLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AcquirerLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Acquirer|array The loaded Acquirer as an assoc-array at the
     *   SDK boundary; throws EvervaultError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EvervaultHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param AcquirerCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AcquirerCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Acquirer|array The created Acquirer as an assoc-array at the
     *   SDK boundary; throws EvervaultError on failure (item-5 convention).
     */
    ...
    /**
     * Update an existing Acquirer.
     *
     * @param AcquirerUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AcquirerUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Acquirer|array The updated Acquirer as an assoc-array at the
     *   SDK boundary; throws EvervaultError on failure (item-5 convention).
     */
