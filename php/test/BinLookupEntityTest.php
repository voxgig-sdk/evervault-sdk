...
class BinLookupEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EvervaultSDK::test(null, null);
        $ent = $testsdk->BinLookup(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = bin_lookup_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "bin_lookup." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_BIN_LOOKUP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $bin_lookup_ref01_ent = $client->BinLookup(null);
...
function bin_lookup_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/bin_lookup/BinLookupTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["bin_lookup01", "bin_lookup02", "bin_lookup03"] as $k) {
    ...
    $entid_env_raw = getenv("EVERVAULT_TEST_BIN_LOOKUP_ENTID");
