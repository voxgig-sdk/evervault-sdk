...
class ClientSideTokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EvervaultSDK::test(null, null);
        $ent = $testsdk->ClientSideToken(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = client_side_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "client_side_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_CLIENT_SIDE_TOKEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $client_side_token_ref01_ent = $client->ClientSideToken(null);
...
function client_side_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/client_side_token/ClientSideTokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["client_side_token01", "client_side_token02", "client_side_token03"] as $k) {
    ...
    $entid_env_raw = getenv("EVERVAULT_TEST_CLIENT_SIDE_TOKEN_ENTID");
