...
class WebhookEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_WEBHOOK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function webhook_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["webhook01", "webhook02", "webhook03", "webhook_endpoint01", "webhook_endpoint02", "webhook_endpoint03"] as $k) {
    ...
    $entid_env_raw = getenv("EVERVAULT_TEST_WEBHOOK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["EVERVAULT_TEST_WEBHOOK_ENTID"]);
