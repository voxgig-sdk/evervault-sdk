...
class NetworkTokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = EvervaultSDK::test(null, null);
        $ent = $testsdk->NetworkToken(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = network_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "network_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_NETWORK_TOKEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $network_token_ref01_ent = $client->NetworkToken(null);
...
function network_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/network_token/NetworkTokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["network_token01", "network_token02", "network_token03"] as $k) {
    ...
    $entid_env_raw = getenv("EVERVAULT_TEST_NETWORK_TOKEN_ENTID");
