...
describe("WebhookEndpointEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_WEBHOOK_ENDPOINT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local webhook_endpoint_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.webhook_endpoint")))
    local webhook_endpoint_ref01_data = nil
    if #webhook_endpoint_ref01_data_raw > 0 then
      webhook_endpoint_ref01_data = helpers.to_map(webhook_endpoint_ref01_data_raw[1][2])
    end

    -- UPDATE
    local webhook_endpoint_ref01_ent = client:WebhookEndpoint(nil)
    local webhook_endpoint_ref01_data_up0_up = {
      id = webhook_endpoint_ref01_data["id"],
    }

    local webhook_endpoint_ref01_markdef_up0_name = "url"
    local webhook_endpoint_ref01_markdef_up0_value = "Mark01-webhook_endpoint_ref01_" .. tostring(setup.now)
    webhook_endpoint_ref01_data_up0_up[webhook_endpoint_ref01_markdef_up0_name] = webhook_endpoint_ref01_markdef_up0_value

    local webhook_endpoint_ref01_resdata_up0_result, err = webhook_endpoint_ref01_ent:update(webhook_endpoint_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local webhook_endpoint_ref01_resdata_up0 = helpers.to_map(type(webhook_endpoint_ref01_resdata_up0_result) == 'table' and webhook_endpoint_ref01_resdata_up0_result.data_get and webhook_endpoint_ref01_resdata_up0_result:data_get() or webhook_endpoint_ref01_resdata_up0_result)
    assert.is_not_nil(webhook_endpoint_ref01_resdata_up0)
    assert.are.equal(webhook_endpoint_ref01_resdata_up0["id"], webhook_endpoint_ref01_data_up0_up["id"])
    assert.are.equal(webhook_endpoint_ref01_resdata_up0[webhook_endpoint_ref01_markdef_up0_name], webhook_endpoint_ref01_markdef_up0_value)
...
function webhook_endpoint_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("EVERVAULT_TEST_WEBHOOK_ENDPOINT_ENTID")
