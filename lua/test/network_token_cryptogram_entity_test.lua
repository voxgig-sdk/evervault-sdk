...
describe("NetworkTokenCryptogramEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:NetworkTokenCryptogram(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = network_token_cryptogram_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "network_token_cryptogram." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set EVERVAULT_TEST_NETWORK_TOKEN_CRYPTOGRAM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local network_token_cryptogram_ref01_ent = client:NetworkTokenCryptogram(nil)
    local network_token_cryptogram_ref01_data = helpers.to_map(vs.getprop(
...
function network_token_cryptogram_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/network_token_cryptogram/NetworkTokenCryptogramTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read network_token_cryptogram test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("EVERVAULT_TEST_NETWORK_TOKEN_CRYPTOGRAM_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["EVERVAULT_TEST_NETWORK_TOKEN_CRYPTOGRAM_ENTID"])
