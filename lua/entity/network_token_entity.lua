...
local NetworkTokenEntity = {}
NetworkTokenEntity.__index = NetworkTokenEntity


function NetworkTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NetworkTokenEntity)
  self._name = "network_token"
  self._client = client
...
function NetworkTokenEntity:get_name()
  return self._name
end


function NetworkTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NetworkTokenEntity.new(self._client, opts)
end
...
function NetworkTokenEntity:mark_deleted()
  self._deleted = true
end


function NetworkTokenEntity:deleted()
  return true == self._deleted
end


function NetworkTokenEntity:data_set(args)
  if args ~= nil then
...
function NetworkTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NetworkTokenEntity:match_set(args)
  if args ~= nil then
...
function NetworkTokenEntity:match_get()
...
function NetworkTokenEntity:stream(action, args, callopts)
...
---@param reqmatch NetworkTokenLoadMatch
---@param ctrl? table
---@return NetworkToken
---@return string? err
function NetworkTokenEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata NetworkTokenCreateData
---@param ctrl? table
---@return NetworkToken
---@return string? err
function NetworkTokenEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function NetworkTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
