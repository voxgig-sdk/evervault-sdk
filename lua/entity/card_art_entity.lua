...
local CardArtEntity = {}
CardArtEntity.__index = CardArtEntity


function CardArtEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardArtEntity)
  self._name = "card_art"
  self._client = client
...
function CardArtEntity:get_name()
  return self._name
end


function CardArtEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardArtEntity.new(self._client, opts)
end
...
function CardArtEntity:mark_deleted()
  self._deleted = true
end


function CardArtEntity:deleted()
  return true == self._deleted
end


function CardArtEntity:data_set(args)
  if args ~= nil then
...
function CardArtEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardArtEntity:match_set(args)
  if args ~= nil then
...
function CardArtEntity:match_get()
...
function CardArtEntity:stream(action, args, callopts)
...
---@param reqmatch CardArtLoadMatch
---@param ctrl? table
---@return CardArt
---@return string? err
function CardArtEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CardArtEntity:_run_op(ctx, post_done)
  local utility = self._utility
