...
local WebhookEndpointEntity = {}
WebhookEndpointEntity.__index = WebhookEndpointEntity


function WebhookEndpointEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhookEndpointEntity)
  self._name = "webhook_endpoint"
  self._client = client
...
function WebhookEndpointEntity:get_name()
  return self._name
end


function WebhookEndpointEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhookEndpointEntity.new(self._client, opts)
end
...
function WebhookEndpointEntity:mark_deleted()
  self._deleted = true
end


function WebhookEndpointEntity:deleted()
  return true == self._deleted
end


function WebhookEndpointEntity:data_set(args)
  if args ~= nil then
...
function WebhookEndpointEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhookEndpointEntity:match_set(args)
  if args ~= nil then
...
function WebhookEndpointEntity:match_get()
...
function WebhookEndpointEntity:stream(action, args, callopts)
...
---@param reqmatch WebhookEndpointLoadMatch
---@param ctrl? table
---@return WebhookEndpoint
---@return string? err
function WebhookEndpointEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata WebhookEndpointUpdateData
---@param ctrl? table
---@return WebhookEndpoint
---@return string? err
function WebhookEndpointEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function WebhookEndpointEntity:_run_op(ctx, post_done)
  local utility = self._utility
