...
local RelayEntity = {}
RelayEntity.__index = RelayEntity


function RelayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RelayEntity)
  self._name = "relay"
  self._client = client
...
function RelayEntity:get_name()
  return self._name
end


function RelayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RelayEntity.new(self._client, opts)
end
...
function RelayEntity:mark_deleted()
  self._deleted = true
end


function RelayEntity:deleted()
  return true == self._deleted
end


function RelayEntity:data_set(args)
  if args ~= nil then
...
function RelayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RelayEntity:match_set(args)
  if args ~= nil then
...
function RelayEntity:match_get()
...
function RelayEntity:stream(action, args, callopts)
...
---@param reqmatch RelayLoadMatch
---@param ctrl? table
---@return Relay
---@return string? err
function RelayEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RelayUpdateData
---@param ctrl? table
---@return Relay
---@return string? err
function RelayEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function RelayEntity:_run_op(ctx, post_done)
  local utility = self._utility
