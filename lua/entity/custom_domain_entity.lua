...
local CustomDomainEntity = {}
CustomDomainEntity.__index = CustomDomainEntity


function CustomDomainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomDomainEntity)
  self._name = "custom_domain"
  self._client = client
...
function CustomDomainEntity:get_name()
  return self._name
end


function CustomDomainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomDomainEntity.new(self._client, opts)
end
...
function CustomDomainEntity:mark_deleted()
  self._deleted = true
end


function CustomDomainEntity:deleted()
  return true == self._deleted
end


function CustomDomainEntity:data_set(args)
  if args ~= nil then
...
function CustomDomainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomDomainEntity:match_set(args)
  if args ~= nil then
...
function CustomDomainEntity:match_get()
...
function CustomDomainEntity:stream(action, args, callopts)
...
---@param reqmatch CustomDomainLoadMatch
---@param ctrl? table
---@return CustomDomain
---@return string? err
function CustomDomainEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CustomDomainCreateData
---@param ctrl? table
---@return CustomDomain
---@return string? err
function CustomDomainEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CustomDomainEntity:_run_op(ctx, post_done)
  local utility = self._utility
