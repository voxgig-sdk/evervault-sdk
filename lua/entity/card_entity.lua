...
local CardEntity = {}
CardEntity.__index = CardEntity


function CardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardEntity)
  self._name = "card"
  self._client = client
...
function CardEntity:get_name()
  return self._name
end


function CardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardEntity.new(self._client, opts)
end
...
function CardEntity:mark_deleted()
  self._deleted = true
end


function CardEntity:deleted()
  return true == self._deleted
end


function CardEntity:data_set(args)
  if args ~= nil then
...
function CardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardEntity:match_set(args)
  if args ~= nil then
...
function CardEntity:stream(action, args, callopts)
...
---@param reqmatch CardLoadMatch
---@param ctrl? table
---@return Card
---@return string? err
function CardEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CardCreateData
---@param ctrl? table
---@return Card
---@return string? err
function CardEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CardEntity:_run_op(ctx, post_done)
  local utility = self._utility
