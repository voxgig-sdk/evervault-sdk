...
local BinLookupEntity = {}
BinLookupEntity.__index = BinLookupEntity


function BinLookupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BinLookupEntity)
  self._name = "bin_lookup"
  self._client = client
...
function BinLookupEntity:get_name()
  return self._name
end


function BinLookupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BinLookupEntity.new(self._client, opts)
end
...
function BinLookupEntity:mark_deleted()
  self._deleted = true
end


function BinLookupEntity:deleted()
  return true == self._deleted
end


function BinLookupEntity:data_set(args)
  if args ~= nil then
...
function BinLookupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BinLookupEntity:match_set(args)
  if args ~= nil then
...
function BinLookupEntity:match_get()
...
function BinLookupEntity:stream(action, args, callopts)
...
---@param reqdata BinLookupCreateData
---@param ctrl? table
---@return BinLookup
---@return string? err
function BinLookupEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function BinLookupEntity:_run_op(ctx, post_done)
  local utility = self._utility
