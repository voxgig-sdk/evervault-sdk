...
local FunctionRunEntity = {}
FunctionRunEntity.__index = FunctionRunEntity


function FunctionRunEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FunctionRunEntity)
  self._name = "function_run"
  self._client = client
...
function FunctionRunEntity:get_name()
  return self._name
end


function FunctionRunEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FunctionRunEntity.new(self._client, opts)
end
...
function FunctionRunEntity:mark_deleted()
  self._deleted = true
end


function FunctionRunEntity:deleted()
  return true == self._deleted
end


function FunctionRunEntity:data_set(args)
  if args ~= nil then
...
function FunctionRunEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FunctionRunEntity:match_set(args)
  if args ~= nil then
...
function FunctionRunEntity:match_get()
...
function FunctionRunEntity:stream(action, args, callopts)
...
---@param reqdata FunctionRunCreateData
---@param ctrl? table
---@return FunctionRun
---@return string? err
function FunctionRunEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FunctionRunEntity:_run_op(ctx, post_done)
  local utility = self._utility
