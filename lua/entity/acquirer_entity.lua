...
local AcquirerEntity = {}
AcquirerEntity.__index = AcquirerEntity


function AcquirerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AcquirerEntity)
  self._name = "acquirer"
  self._client = client
...
function AcquirerEntity:get_name()
  return self._name
end


function AcquirerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AcquirerEntity.new(self._client, opts)
end
...
function AcquirerEntity:mark_deleted()
  self._deleted = true
end


function AcquirerEntity:deleted()
  return true == self._deleted
end


function AcquirerEntity:data_set(args)
  if args ~= nil then
...
function AcquirerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AcquirerEntity:match_set(args)
  if args ~= nil then
...
function AcquirerEntity:match_get()
...
function AcquirerEntity:stream(action, args, callopts)
...
---@param reqmatch AcquirerLoadMatch
---@param ctrl? table
---@return Acquirer
---@return string? err
function AcquirerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AcquirerCreateData
---@param ctrl? table
---@return Acquirer
---@return string? err
function AcquirerEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata AcquirerUpdateData
---@param ctrl? table
---@return Acquirer
---@return string? err
function AcquirerEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AcquirerEntity:_run_op(ctx, post_done)
  local utility = self._utility
