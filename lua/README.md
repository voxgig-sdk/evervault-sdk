...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Card()` — each with the same small set of operations (`load`, `create`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
-- Create
local created, err = client:Card():create({ expiry = {}, month = "example_month", number = "example_number", year = "example_year" })
if err then error(err) end
