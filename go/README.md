...
It exposes the API as capitalised, semantic **Entities** — e.g. `client.Card(nil)` — each with the same small set of operations (`Load`, `Create`, `Remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
func main() {
    ...
    // Create a card.
    created, err := client.Card(nil).Create(map[string]any{"expiry": map[string]any{}, "month": "example_month", "number": "example_number", "year": "example_year"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(created)
