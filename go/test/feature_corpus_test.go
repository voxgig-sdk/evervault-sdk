...
//
// NewEvervaultSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.EvervaultSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewEvervaultSDK(opts)
}
...
func fcCandidates(client *sdk.EvervaultSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.EvervaultSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.EvervaultSDK, name string) any {
