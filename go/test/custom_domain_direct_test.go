...
func TestCustomDomainDirect(t *testing.T) {
	t.Run("direct-load-custom_domain", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-custom_domain", _mode); _shouldSkip {
			if _reason == "" {
...
type custom_domainDirectSetupResult struct {
...
func custom_domainDirectSetup(mockres any) *custom_domainDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"EVERVAULT_TEST_CUSTOM_DOMAIN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["EVERVAULT_TEST_CUSTOM_DOMAIN_ENTID"]; ok {
		...
		return &custom_domainDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &custom_domainDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
