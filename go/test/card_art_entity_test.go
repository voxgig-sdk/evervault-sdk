...
func TestCardArtEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		...
		if ent == nil {
			t.Fatal("expected non-nil CardArtEntity")
		}
	})
	...
	t.Run("basic", func(t *testing.T) {
		...
		cardArtRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.card_art", setup.data)))
		var cardArtRef01Data map[string]any
		if len(cardArtRef01DataRaw) > 0 {
			cardArtRef01Data = core.ToMapAny(cardArtRef01DataRaw[0][1])
		}
...
func card_artBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "card_art", "CardArtTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read card_art test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse card_art test data: " + err.Error())
	}
	...
	idmap := vs.Transform(
		[]any{"card_art01", "card_art02", "card_art03", "network_token01", "network_token02", "network_token03"},
		map[string]any{
	...
	idmapResolved := core.ToMapAny(env["EVERVAULT_TEST_CARD_ART_ENTID"])
