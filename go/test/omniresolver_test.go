...
import (
	...
	omni "github.com/voxgig-sdk/evervault-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EvervaultError); is {
				out := map[string]any{
					"name":    "EvervaultError",
...
func omniCtx(arg any, client *sdk.EvervaultSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EvervaultSDK); is {
			client = live
