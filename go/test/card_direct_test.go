...
func TestCardDirect(t *testing.T) {
	t.Run("direct-load-card", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-card", _mode); _shouldSkip {
			if _reason == "" {
...
func cardDirectSetup(mockres any) *cardDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"EVERVAULT_TEST_CARD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["EVERVAULT_TEST_CARD_ENTID"]; ok {
